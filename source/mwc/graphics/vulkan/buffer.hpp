  ...
  namespace graphics {
    namespace vulkan {
      class buffer_ct : public handle_ct<vk::raii::Buffer> {
        ...
        auto allocation_info(this tp_this&& a_this) -> decltype(auto);
        template <typename tp_this>
        auto allocation(this tp_this&& a_this) -> decltype(auto);

        auto device_address_requested() const -> bool_t;
        auto address() const -> vk::DeviceAddress pre(device_address_requested()) post(r : r != vk::DeviceAddress {0});
        auto size() const -> vk::DeviceSize post(r : r != vk::DeviceSize {0});
