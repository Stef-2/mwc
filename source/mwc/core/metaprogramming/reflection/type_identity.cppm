...
export namespace mwc {
  namespace meta {
    ...
    // unary predicate to be passed to [meta::search()] to locate all types that crtp-inherit from [type_index_st]
    // has to be a template to force the compiler to reevaluate it in each context in which it appears
    template <typename tp>
    constexpr auto indexing_unary_predicate = [](const std::meta::info a_info) consteval -> bool_t {
      if (std::meta::is_complete_type(a_info)) {
        // the default meta::search() engine already has a safeguard against parsing namespace std contents
        // note: consider removing this if that turns out to be a permanent decision
    ...
    // index generator
    // works by using [meta::search()] to find all types that crtp-inherit from [type_index_st]
    // then returns the distance between [tp] and the beginning of the reflected type range
    template <typename tp>
    constexpr auto index() -> size_t {
      constexpr auto descendents = meta::search(indexing_unary_predicate<tp>);
