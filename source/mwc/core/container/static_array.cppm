...
export namespace mwc {
  ...
  struct static_array_st {
    ...
    constexpr static_array_st(const tp (&a_data)[tp_count]) pre(tp_count > 0 and tp_count < std::numeric_limits<size_t>::max()) : m_data {std::to_array(a_data)} {}

    constexpr static_array_st(const span_t<const tp, tp_count> a_data) pre(contract::validate_storage(a_data)): m_data {} {
