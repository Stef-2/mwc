...
namespace mwc {
  struct semantic_version_st {
    ...
    constexpr semantic_version_st(const packed_version_t a_packed_version) pre(a_packed_version != 0)
      pre(a_packed_version <= std::numeric_limits<packed_version_t>::max());
    constexpr semantic_version_st(const version_t a_major, const version_t a_minor, const version_t a_patch)
    ...
    auto operator<=>(const semantic_version_st&) const = default;

    packed_version_t m_version {};
  };

  // implementation
  constexpr semantic_version_st::semantic_version_st(const packed_version_t a_packed_version) : m_version {a_packed_version} {}
  constexpr semantic_version_st::semantic_version_st(const version_t a_major, const version_t a_minor, const version_t a_patch)
