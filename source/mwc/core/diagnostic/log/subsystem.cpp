...
namespace mwc {
    ...
    namespace log {
      auto log_subsystem_st::initialize() -> void {
        ...
        auto sinks = array_t<sink_st, sink_count> {file_ptr_t {nullptr}, file_ptr_t {nullptr},    file_ptr_t {nullptr},
                                                   file_ptr_t {nullptr}, ostream_ptr_t {nullptr}, string_ptr_t {nullptr}};
        ...
        for (auto i = std::underlying_type_t<event_severity_et> {0}; i < std::to_underlying(event_severity_et::end); ++i) {
          ...
          constexpr auto log_file_configuration
            = configuration_st {std::to_underlying(e_print_timestamp bitor e_print_severity_level bitor e_print_thread_id
                                                   bitor e_print_source_location bitor e_print_stacktrace)};
          sinks[i] = {log_files[i], event_severity_et {i}, log_file_configuration};
        }
        // standard output sink
        constexpr auto standard_stream_configuration
          = configuration_st {std::to_underlying(e_print_timestamp bitor e_print_severity_level bitor e_cover_higher_severities)};
        std::println("[{0}] {1} at address {2}", sink_count - 2, "standard log output", static_cast<void*>(&output::log_stream()));
        sinks[4] = {&output::log_stream(), event_severity_et::e_information, standard_stream_configuration};
        // string sink
        std::println("[{0}] {1} at address {2}", sink_count - 1, "string output", static_cast<void*>(string_sink.data()));
        sinks[5] = {&string_sink, event_severity_et::e_information, standard_stream_configuration};

        if constexpr (logging_subsystem_switch())
          log = log_ct {sinks};
        else
          log = log_ct {};
  ...
  auto information<true>(const string_view_t a_message, const std::source_location& a_source_location)
    ...
    diagnostic::log::log_subsystem_st::log.information(a_message, a_source_location);
  }
  template <>
  ...
  namespace diagnostic {
    namespace log {
      auto log_subsystem_st::finalize() -> void {
        for (const auto& sink : log.storage())
